...
object warehouseOrders {
  ...
  /*def updateOrderStatus(ordId: Int){
    ...
    def main(args: Array[String]) {
      ...
      var printList = readLine("Would you like to print a list of orders?")
      ...
      for(i <- 0 to orders1.length - 1) {
        if(orders1(i).orderId.toString == orderNo) {
            println("OrderID: " + orders1(i).orderId + ", DateTime: " + orders1(i).dateTime + ", CustomerID: " + orders1(i).customerId + ", Status: " + orders1(i).status)
        } else {
